...
- [ ] Nothing in the CLI or TUI displays `DrugFact`s (contraindications, boxed warnings, etc.), RxCUI, chemical properties, or ATC codes yet, even once a build-time-enriched database is installed: neither interface has any screen for a single drug's own facts or cross-reference data today, only for a pair's interaction. This is real, separate display design work (a new screen or section, not an extension of the existing interaction result), not attempted here
