...
- [ ] OpenFDA, RxNorm, PubChem, and ATC are not wired into `mensung-client`'s runtime install flow, and should not be: at DDInter's real ~1900-drug scale, running all four rate-limited fetchers live, once per end-user install, is roughly an hour of sequential HTTPS requests (OpenFDA alone, paced at 40 requests/minute, is about 50 minutes by itself), against exactly the low-connectivity field deployments MenSung targets. A single failed request partway through an hour-long install is a much worse failure mode than DDInter's current few-second fetch. The right shape for this data is the same one DDInter's own mirror already uses: compiled once, offline, by a maintainer or CI job, and published as a `.men` file release asset that `mensung-client` downloads as a single file, the way it already downloads DDInter's raw CSVs and compiles them locally today, or closer to that mirror's own "fetch a finished artifact" model. This needs the builder CLI Phase 5 already flagged as "not needed yet" -- it is needed now, to run this enrichment pipeline once, not per installation. Not built yet; runtime per-user fetching of these four sources is not the target design, not merely an unfinished version of it
